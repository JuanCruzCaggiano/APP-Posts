<x-layouts.app :title="$post->title" :meta-description="$post->body">
    ...
    <form class="max-w-xl px-8 py-4 mx-auto bg-white rounded shadow dark:bg-slate-800" action="{{route('posts.update', $post)}}" method="POST">
        @csrf <!--para que no arroje error 419 al submitear un formulario-->
        @method('PATCH')<!-- el metodo patch no esta disponible para forms, agregamos esto para poder actualizarlo-->
